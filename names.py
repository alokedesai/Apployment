...
for datum in data:
    ...
    for school in schools:
		...
		year="2016"
	    if 'major' in school:
  		major=school["concentration"]["name"]
	    else:
		major = "Undeclared"
        else : 
	    college = ""
    ...
    wage = wage%40+10
    user = User(username=datum["name"].replace(" ",""),email=datum["name"].replace(" ","")+"@apployment.com",first_name=n[0],last_name=n[len(n)-1],school=college,grad_year=year,major=major,description="Filler",gpa=random.uniform(2.0,4.0),school_location="USA",school_type="University",rank=rank,wage=wage)
    user.set_password("password")
    user.save()
    
    x = hasSkill(user=user,skill=Skill.objects.all()[random.randint(0,299)])
    x.save()
